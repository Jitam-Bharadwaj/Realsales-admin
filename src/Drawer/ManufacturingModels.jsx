...
const ManufacturingModels = ({ currentSegment }) => {
  ...
  const [loadingManufacturingModels, setLoadingManufacturingModels] =
    useState(false);
  ...
  const readManufacturingModels = async () => {
    ...
    try {
      let data = await axioInstance.get(endpoints.ai.manufacturing_models);
      if (data?.data?.length) {
        setManufacturingModels(data?.data);
      } else {
        setManufacturingModels([]);
      }
    } catch (error) {
      setManufacturingModels([]);
      console.log(error, "_error_");
    } finally {
      setLoadingManufacturingModels(false);
    }
  };

  const createManufacturingModels = async () => {
    setLoading(true);
    try {
      let data = await axioInstance.post(endpoints.ai.manufacturing_models, {
        name: editingData?.name,
        description: convertNewlines(editingData?.description),
      });
      if (data?.data?.manufacturing_model_id) {
  ...
  const updateManufacturingModels = async () => {
    setLoading(true);
    try {
      let data = await axioInstance.put(
        `${endpoints.ai.manufacturing_models}${editingData?.id}`,
        {
          name: editingData?.name,
          description: convertNewlines(editingData?.description),
        }
  ...
  const deleteManufacturingModels = async (id) => {
    try {
      let data = await axioInstance.delete(
        `${endpoints.ai.manufacturing_models}${id}`
      );
      if (data?.status === 204) {
        readManufacturingModels();
        setDeleteId({});
  ...
  const validateManufacturingModel = (data) => {
    const errors = {};
  ...
  return (
    <>
      {/* role */}
      {currentSegment === "manufacturingModels" && (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    const errors = validateManufacturingModel(editingData);
                    ...
                    setValidationError({});
                    if (editingData?.id) {
                      updateManufacturingModels();
                    } else {
                      createManufacturingModels();
                    }
                  }}
                  disabled={loading}
                >
            ...
            <>
              <Table>
                ...
                <TableBody>
                  ...
                  ) : manufacturingModels?.length ? (
                    manufacturingModels.map((v, i) => (
                      <TableRow key={i}>
          ...
          <Dialog open={deleteId?.id} onClose={() => setDeleteId({})}>
            ...
            <DialogActions>
              ...
              <Button
                ...
                onClick={() => deleteManufacturingModels(deleteId?.id)}
                autoFocus
              >
                Delete
              </Button>
